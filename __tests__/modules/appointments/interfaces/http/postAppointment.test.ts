...
vi.mock(
  '@/modules/appointments/infrastructure/messaging/SNSMessagePublisher.js'
);
...
describe('postAppointment Handler', () => {
  ...
  it('should handle application errors', async () => {
    ...
    mockCreateAppointment.execute.mockRejectedValue(
      new TestAppError('Appointment slot already taken')
    );
  ...
  it('should initialize dependencies correctly', async () => {
    ...
    expect(SNSMessagePublisher).toHaveBeenCalledWith({
      topicArn: 'arn:aws:sns:us-east-1:123456789012:appointment-notifications',
      region: 'us-east-1',
    });
  });
});
