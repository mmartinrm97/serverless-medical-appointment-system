...
vi.mock(
  '@/modules/appointments/application/use-cases/ListAppointmentsByInsured.js'
);
vi.mock(
  '@/modules/appointments/infrastructure/db/dynamodb/DynamoDBAppointmentsRepository.js'
);
...
describe('getAppointments Handler', () => {
  ...
  beforeEach(() => {
    ...
    (ListAppointmentsByInsured as any).mockImplementation(
      () => mockListAppointments
    );
  ...
  it('should retrieve appointments successfully', async () => {
    ...
    expect(responseBody.appointments[0].appointmentId).toBe(
  ...
  it('should handle application errors', async () => {
    ...
    mockListAppointments.execute.mockRejectedValue(
      new TestAppError('Database unavailable')
    );
