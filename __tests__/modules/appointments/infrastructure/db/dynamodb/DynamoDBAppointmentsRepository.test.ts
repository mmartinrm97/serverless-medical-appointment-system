...
import {
  Appointment,
  type CountryISO,
} from '@/modules/appointments/domain/entities/Appointment.js';
import {
...
describe('DynamoDBAppointmentsRepository', () => {
  ...
  beforeEach(() => {
    ...
    mockSend = vi.fn();
    const MockDynamoDBDocumentClient = DynamoDBDocumentClient as any;
    MockDynamoDBDocumentClient.from.mockReturnValue({
      send: mockSend,
    });

    repository = new DynamoDBAppointmentsRepository({
      region: 'us-east-1',
      tableName,
    });
  });
  ...
  describe('save()', () => {
    it('should save appointment successfully', async () => {
      // Arrange
      mockSend.mockResolvedValue(mockDynamoDBPutResponse);
    ...
    it('should handle save errors', async () => {
      // Arrange
      mockSend.mockRejectedValue(mockDynamoDBError);
      const appointment = Appointment.create({
        insuredId: '12345',
        scheduleId: 100,
        countryISO: 'PE' as CountryISO,
  ...
  describe('findById()', () => {
    it('should find appointment by id successfully', async () => {
      ...
      mockSend.mockResolvedValue(queryResponse);
    ...
    it('should return null when appointment not found', async () => {
      // Arrange
      mockSend.mockResolvedValue({ Items: [], Count: 0, ScannedCount: 0 });
  ...
  describe('findByInsuredId()', () => {
    it('should find appointments by insured id successfully', async () => {
      // Arrange
      mockSend.mockResolvedValue(mockDynamoDBQueryResponse);
    ...
    it('should handle single appointment result', async () => {
      ...
      mockSend.mockResolvedValue(singleItemResponse);
  ...
  describe('Domain entity conversion', () => {
    it('should correctly convert DynamoDB item to domain entity', async () => {
      ...
      expect(result?.slotDatetime).toEqual(
