...
import { InfrastructureError, ConflictError } from '../../../../../shared/domain/errors/index.js';
import { createLogger } from '../../../../../shared/infrastructure/logging/logger.js';
import { AppointmentMapper, type DynamoDBAppointmentItem } from './AppointmentMapper.js';
...
export class DynamoDBAppointmentsRepository implements IAppointmentsRepository {
  ...
  private readonly logger = createLogger({ component: 'DynamoDBAppointmentsRepository' });
  ...
  async save(appointment: Appointment): Promise<void> {
    try {
      ...
      this.logger.info(`Appointment saved successfully: ${appointment.appointmentId} for insured ${appointment.insuredId}`);

    } catch (error: unknown) {
      if (error instanceof Error && error.name === 'ConditionalCheckFailedException') {
        this.logger.warn(`Appointment already exists: ${appointment.appointmentId} for insured ${appointment.insuredId}`);
        throw new ConflictError('appointment', 'Appointment already exists');
      }

      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to save appointment: ${errorMessage} for appointment ${appointment.appointmentId}`);
  ...
  async findById(appointmentId: string): Promise<Appointment | null> {
    try {
      ...
      const appointment = this.mapper.toEntity(result.Items[0] as DynamoDBAppointmentItem);
      this.logger.debug(`Appointment found: ${appointmentId}`);
      return appointment;

    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to find appointment by ID: ${appointmentId} - ${errorMessage}`);
  ...
  async findByInsuredId(insuredId: string): Promise<Appointment[]> {
    try {
      ...
      const appointments = result.Items.map(item => this.mapper.toEntity(item as DynamoDBAppointmentItem));

      this.logger.debug(`Found ${appointments.length} appointments for insured: ${insuredId}`);

      return appointments;

    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to find appointments by insured ID: ${insuredId} - ${errorMessage}`);
      throw new InfrastructureError('find appointments by insured ID', errorMessage);
    }
  }
   ...
   */
  async updateStatus(appointmentId: string, status: 'completed' | 'failed'): Promise<Appointment | null> {
    try {
      // First, we need to find the appointment to get the PK (insuredId)
      const appointment = await this.findById(appointmentId);

      if (!appointment) {
        this.logger.warn(`Appointment not found for status update: ${appointmentId}`);
        return null;
      }
      ...
      const updatedAppointment = this.mapper.toEntity(result.Attributes as DynamoDBAppointmentItem);

      this.logger.info(`Appointment status updated: ${appointmentId} to ${status}`);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to update appointment status: ${appointmentId} - ${errorMessage}`);
   ...
   * @throws {InfrastructureError} When query operation fails
   */
  async findByInsuredAndSchedule(insuredId: string, scheduleId: number): Promise<Appointment | null> {
    try {
      const command = new QueryCommand({
        TableName: this.tableName,
        KeyConditionExpression: 'PK = :insuredId',
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to find appointment by insured and schedule: ${insuredId}, ${scheduleId} - ${errorMessage}`);
      throw new InfrastructureError('find appointment by insured and schedule', errorMessage);
    }
  }
}
