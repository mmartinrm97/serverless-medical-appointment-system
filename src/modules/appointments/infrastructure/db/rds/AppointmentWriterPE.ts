...
export class AppointmentWriterPE implements ICountryStrategy {
  ...
  async processAppointment(
    ...
    this.logger.info(
      `Processing appointment for Peru: ${appointment.appointmentId}`
    );
    ...
    try {
      ...
      try {
        ...
        this.logger.info(
          `Appointment successfully written to Peru RDS: ${appointment.appointmentId}, record ID: ${rdsRecordId}`
        );
      ...
      this.logger.error(
        `Failed to process appointment in Peru: ${errorMessage} for appointment ${appointment.appointmentId}`
      );
  ...
  async validateAppointment(appointment: Appointment): Promise<boolean> {
    try {
      ...
      if (appointment.countryISO !== 'PE') {
        this.logger.warn(
          `Invalid country for Peru writer: ${appointment.countryISO}`
        );
        return false;
      }
      ...
      if (!/^\d{5}$/.test(appointment.insuredId)) {
        this.logger.warn(
          `Invalid insured ID format for Peru: ${appointment.insuredId}`
        );
        return false;
      }
      ...
      if (appointment.scheduleId <= 0) {
        this.logger.warn(
          `Invalid schedule ID for Peru: ${appointment.scheduleId}`
        );
        return false;
      }

      this.logger.debug(
        `Appointment validation passed for Peru: ${appointment.appointmentId}`
      );
      return true;
    } catch (error: unknown) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Appointment validation failed for Peru: ${errorMessage}`
      );
      return false;
    }
  }
