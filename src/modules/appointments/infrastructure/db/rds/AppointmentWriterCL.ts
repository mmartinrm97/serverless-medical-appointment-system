...
export class AppointmentWriterCL implements ICountryStrategy {
    ...
    async processAppointment(appointment: Appointment): Promise<AppointmentProcessResult> {
        this.logger.info(`Processing appointment for Chile: ${appointment.appointmentId}`);
        ...
        `;
                ...
                this.logger.info(`Appointment successfully written to Chile RDS: ${appointment.appointmentId}, record ID: ${rdsRecordId}`);
            ...
            this.logger.error(`Failed to process appointment in Chile: ${errorMessage} for appointment ${appointment.appointmentId}`);
    ...
    async validateAppointment(appointment: Appointment): Promise<boolean> {
        try {
            ...
            if (appointment.countryISO !== 'CL') {
                this.logger.warn(`Invalid country for Chile writer: ${appointment.countryISO}`);
                return false;
            }

            // Validate insured ID format for Chile (5 digits)
            if (!/^\d{5}$/.test(appointment.insuredId)) {
                this.logger.warn(`Invalid insured ID format for Chile: ${appointment.insuredId}`);
                return false;
            }

            // Validate schedule ID is positive
            if (appointment.scheduleId <= 0) {
                this.logger.warn(`Invalid schedule ID for Chile: ${appointment.scheduleId}`);
                return false;
            }
            ...
            this.logger.debug(`Appointment validation passed for Chile: ${appointment.appointmentId}`);
            return true;

        } catch (error: unknown) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Appointment validation failed for Chile: ${errorMessage}`);
            return false;
        }
    }
