...
export class RDSClient {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info(
        `RDS connection pool created successfully for ${this.config.host}`
      );
    } catch (error: unknown) {
      const errorMessage =
  ...
  async execute<T = mysql.RowDataPacket[]>(
    ...
    while (retries > 0) {
      try {
        ...
        this.logger.debug(
          `Query executed successfully: ${query.substring(0, 100)}...`
        );
        return rows as T;
      } catch (error: unknown) {
        ...
        if (this.isRetryableError(lastError) && retries > 0) {
          this.logger.warn(
            `Retryable error occurred, retries left: ${retries} - ${lastError.message}`
          );
...
export class RDSTransaction {
  ...
  async execute<T = mysql.RowDataPacket[]>(
    ...
    try {
      ...
      this.logger.debug(
        `Transaction query executed: ${query.substring(0, 100)}...`
      );
      return rows as T;
    } catch (error: unknown) {
      const errorMessage =
