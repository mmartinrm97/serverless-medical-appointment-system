...
export class EventBridgePublisher implements IEventPublisher {
  ...
  constructor(
    ...
    this.logger.info(
      `EventBridge publisher initialized for bus: ${this.eventBusName}`
    );
  }
  ...
  async publishAppointmentConfirmed(
    ...
    try {
      ...
      this.logger.info(
        `Published appointment confirmed event: ${event.detail.appointmentId} to EventBridge`
      );
    } catch (error: unknown) {
      ...
      this.logger.error(
        `Failed to publish event to EventBridge: ${errorMessage} for appointment ${event.detail.appointmentId}`
      );
      throw new InfrastructureError('EventBridge publish', errorMessage);
    }
  }
  ...
  async publishEvent(eventData: {
    ...
    try {
      ...
      if (result.FailedEntryCount && result.FailedEntryCount > 0) {
        ...
        this.logger.error(
          `EventBridge generic publish failed: ${errorMessage}`
        );
        throw new InfrastructureError('EventBridge publish', errorMessage);
      }

      this.logger.info(
        `Published generic event to EventBridge: source=${eventData.source}, detailType=${eventData.detailType}`
      );
    } catch (error: unknown) {
      if (error instanceof InfrastructureError) {
        throw error;
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Failed to publish generic event to EventBridge: ${errorMessage}`
      );
      throw new InfrastructureError('EventBridge publish', errorMessage);
    }
  }
