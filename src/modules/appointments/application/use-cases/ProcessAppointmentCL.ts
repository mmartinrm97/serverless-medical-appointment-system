...
export class ProcessAppointmentCL {
     ...
     */
    public async execute(sqsMessage: AppointmentCLSQSMessage): Promise<ProcessAppointmentCLResult> {
        ...
        try {
            ...
            if (countryISO !== 'CL') {
                throw new ValidationError(
                    'Invalid country for Chile processor',
            ...
            try {
                ...
                const processResult = await chileStrategy.processAppointment(appointment);
                ...
                rdsWritten = true;
                logger.info(`Successfully wrote appointment ${appointmentId} to Chile RDS`);

            } catch (error) {
                logger.error(`Failed to write appointment ${appointmentId} to Chile RDS: ${error}`);
                throw new InfrastructureError(
                    'Chile RDS write',
            ...
            logger.error(`Failed to process Chile appointment ${appointmentId}: ${error}`);
    ...
    private validateSQSMessage(message: AppointmentCLSQSMessage): void {
        ...
        for (const field of requiredFields) {
            if (!(field in message) || !message[field as keyof AppointmentCLSQSMessage]) {
                throw new ValidationError(
    ...
    private reconstructAppointmentFromMessage(message: AppointmentCLSQSMessage): Appointment {
