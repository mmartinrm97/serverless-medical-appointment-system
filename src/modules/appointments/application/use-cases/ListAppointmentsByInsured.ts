...
export class ListAppointmentsByInsured {
    ...
    public async execute(insuredId: string): Promise<AppointmentListResponseDto> {
        ...
        try {
            ...
            const appointments = await this.appointmentsRepository.findByInsuredId(insuredId);

            logger.info(`Found ${appointments.length} appointments for insured: ${insuredId}`);

            // Step 3: Convert entities to DTOs and create response
            const appointmentProps = appointments.map(appointment => appointment.toProps());
            ...
            const statusCounts = appointmentProps.reduce((acc, appointment) => {
                acc[appointment.status] = (acc[appointment.status] || 0) + 1;
                return acc;
            }, {} as Record<string, number>);

            logger.info(`Appointment status summary for ${insuredId}: ${JSON.stringify(statusCounts)}`);

            return response;

        } catch (error) {
            logger.error(`Failed to list appointments for insured ${insuredId}: ${error}`);
            throw error;
        }
    }
    ...
    public async executeWithPagination(
        ...
        logger.info(`Listing appointments with pagination for insured: ${insuredId} - options: ${JSON.stringify(options)}`);
        ...
        try {
            ...
            const allAppointments = await this.appointmentsRepository.findByInsuredId(insuredId);
            ...
            logger.info(`Returning page ${page} with ${paginatedAppointments.length} appointments`);

            // Step 4: Create response with pagination info
            const appointmentProps = paginatedAppointments.map(appointment => appointment.toProps());

            return createAppointmentListResponse(
                appointmentProps,
                insuredId,
                {
                    page,
            ...
            logger.error(`Failed to list appointments with pagination for insured ${insuredId}: ${error}`);
            throw error;
        }
    }
}
