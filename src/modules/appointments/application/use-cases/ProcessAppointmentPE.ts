...
export class ProcessAppointmentPE {
  ...
  public async execute(
    sqsMessage: AppointmentSQSMessage
  ): Promise<ProcessAppointmentPEResult> {
    ...
    try {
      ...
      if (countryISO !== 'PE') {
        throw new ValidationError('Invalid country for Peru processor', {
          appointmentId,
          countryISO,
        });
      }
      ...
      try {
        ...
        const processResult =
          await peruStrategy.processAppointment(appointment);
        ...
        rdsWritten = true;
        logger.info(
          `Successfully wrote appointment ${appointmentId} to Peru RDS`
        );
      } catch (error) {
        logger.error(
          `Failed to write appointment ${appointmentId} to Peru RDS: ${error}`
        );
        throw new InfrastructureError(
          'Peru RDS write',
      ...
      logger.error(
        `Failed to process Peru appointment ${appointmentId}: ${error}`
      );
  ...
  private validateSQSMessage(message: AppointmentSQSMessage): void {
    ...
    for (const field of requiredFields) {
      if (
        !(field in message) ||
        !message[field as keyof AppointmentSQSMessage]
      ) {
