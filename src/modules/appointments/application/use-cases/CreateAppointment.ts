...
export class CreateAppointment {
     ...
     * ```
     */
    public async execute(request: CreateAppointmentRequest): Promise<CreateAppointmentResponse> {
        logger.info(`Creating appointment for insured ${request.insuredId}, schedule ${request.scheduleId}`);
        ...
        try {
            ...
            const existingAppointment = await this.appointmentsRepository.findByInsuredAndSchedule(
                request.insuredId,
                request.scheduleId
            );

            if (existingAppointment) {
                logger.info(`Duplicate appointment found for insured ${request.insuredId}, schedule ${request.scheduleId}`);
            ...
            logger.info(`Created appointment entity with ID: ${appointment.appointmentId}`);

            // Step 4: Save appointment to DynamoDB (status: "pending")
            try {
                await this.appointmentsRepository.save(appointment);
                logger.info(`Saved appointment to DynamoDB: ${appointment.appointmentId}`);
            } catch (error) {
            ...
            try {
                ...
                logger.info(`Published SNS message for appointment: ${appointment.appointmentId}`);
            } catch (error) {
            ...
            logger.info(`Successfully created appointment: ${appointment.appointmentId}`);
            return response;

        } catch (error) {
            logger.error(`Create appointment failed for insured ${request.insuredId}: ${error}`);
            throw error;
        }
    }
     ...
     */
    private async publishAppointmentMessage(appointment: Appointment): Promise<void> {
