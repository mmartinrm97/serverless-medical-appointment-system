...
} from '../../domain/events/AppointmentConfirmed.js';
import {
  NotFoundError,
  ValidationError,
...
export class ConfirmAppointment {
  ...
  public async executeFromEvent(
    ...
    try {
      ...
      if (!validateAppointmentConfirmedData(appointmentConfirmedEvent.detail)) {
        throw new ValidationError('Invalid event detail structure', {
      ...
      logger.error(
        `Failed to process appointment confirmation event: ${error}`
      );
      throw error;
    }
  }
  ...
  public async execute(confirmationData: {
    ...
    logger.info(
      `Confirming appointment ${appointmentId} processed by ${source}`
    );
    ...
    try {
      ...
      if (appointment.insuredId !== confirmationData.insuredId) {
        throw new ValidationError('Appointment insured ID mismatch', {
          appointmentId,
      ...
      logger.info(
        `Successfully confirmed appointment ${appointmentId}: ${previousStatus} -> completed`
      );
  ...
  public async executeBatch(
    confirmationEvents: unknown[]
  ): Promise<ConfirmAppointmentResult[]> {
    logger.info(
      `Processing batch of ${confirmationEvents.length} confirmation events`
    );
    ...
    logger.info(
      `Batch processing completed: ${results.length} success, ${errors.length} errors`
    );
    ...
    if (errors.length > 0) {
      logger.error(
        `Batch processing had ${errors.length} errors: ${JSON.stringify(errors)}`
      );
    }
