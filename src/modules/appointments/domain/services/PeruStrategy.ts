...
export class PeruStrategy implements ICountryStrategy {
  ...
  public async processAppointment(
    ...
    try {
      ...
      logger.info(
        `Appointment successfully processed for Peru: ${appointment.appointmentId}`
      );
      ...
      logger.error(
        `Failed to process appointment for Peru: ${appointment.appointmentId} - ${error}`
      );
  ...
  private async writeToPeruDatabase(appointment: Appointment): Promise<void> {
    ...
    logger.debug(
      `Simulated Peru database write for appointment: ${appointment.appointmentId}`
    );
