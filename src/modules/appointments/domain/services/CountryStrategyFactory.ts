...
export class CountryStrategyFactory {
    private static readonly strategies = new Map<CountryISO, () => ICountryStrategy>([
        ['PE', () => new PeruStrategy()],
    ...
    public static create(countryISO: CountryISO): ICountryStrategy {
        ...
        if (!strategyFactory) {
            throw new ValidationError(
                `Unsupported country: ${countryISO}`,
                { countryISO, supportedCountries: Array.from(this.strategies.keys()) }
            );
        }
