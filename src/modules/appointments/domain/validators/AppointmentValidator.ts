...
export class AppointmentValidator {
  ...
  public static validateInsuredId(insuredId: string): void {
    if (!insuredId || typeof insuredId !== 'string') {
      throw new ValidationError('Insured ID is required and must be a string', {
        insuredId,
      });
    }
    ...
    if (!/^\d{5}$/.test(insuredId)) {
      throw new ValidationError('Insured ID must be exactly 5 digits', {
        insuredId,
        format: 'Expected: 5 digits (e.g., "12345")',
      });
    }
  }
  ...
  public static validateScheduleId(scheduleId: number): void {
    if (typeof scheduleId !== 'number' || !Number.isInteger(scheduleId)) {
      throw new ValidationError('Schedule ID must be an integer', {
        scheduleId,
   ...
   */
  public static validateCountryISO(
    countryISO: string
  ): asserts countryISO is CountryISO {
  ...
  public static sanitizeInsuredId(insuredId: string): string {
    ...
    if (!/^\d+$/.test(trimmed)) {
      throw new ValidationError('Insured ID must contain only digits', {
        insuredId,
        trimmed,
      });
    }
    ...
    if (trimmed.length > 5) {
      throw new ValidationError('Insured ID cannot be longer than 5 digits', {
        insuredId,
